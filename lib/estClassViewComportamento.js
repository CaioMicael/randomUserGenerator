export class estClassViewComportamento {
    ...
    async doAjaxTelaInclusao(caminhoAjax,dados) {
        try {
            const telaInclusao = await this.getTelaInclusao(caminhoAjax,dados);
    ...
    async getTelaInclusao(caminho,dados) {
        try {
            const resposta = await this.doAjaxCarrega(caminho,dados);
            return resposta;
        } catch (error) {
