...
export class estClassViewComponentes {
   ...
   destacaInputVazio(xComponente) {
    ...
    /**
     * Este método retorna o button de confirmar alteração.
     * @returns element
     */
    getComponenteButtonConfirmarAlteracao() {
        return document.querySelector(".estButtonConfirmarAlteracao");
    }
   ...
   getAllComponenteCheckboxTelaSelecionar() {
    ...
    /**
     * Este metodo retorna um nodeList com os table header.
     * @returns {NodeList}
     */
    getAllComponenteTableHeader()  {
        const allHeaders = document.querySelectorAll('th:not(:has(id))');
        const filteredHeaders = Array.from(allHeaders).filter(th => th.innerHTML.trim() !== '' && th.id === '');
    ...
    /**
      * Este método retorna um array com os valores dos checkbox
     ...
     getValoresComponenteCheckboxTableChecked() {
        ...
        var aTH  = this.getAllComponenteTableHeader();
        ...
        const aDados = aTH.map((aTH, index) => [aTH.innerHTML, values[index]]);
        return aDados;
     }
}
